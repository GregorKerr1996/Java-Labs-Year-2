...
public class GameTester {
	...
	public void testHumanPlayerShouldReadFromStdin() {
		...
		Scanner scanner = new Scanner(Symbol.ROCK.toString());

		HumanPlayer hp = new HumanPlayer("Fred", scanner);
		Symbol humanSymbol = hp.chooseSymbol();

		Assert.assertFalse("HumanPlayer.chooseSymbol() should prompt to stdout", baos.toString().isEmpty());
		Assert.assertEquals("Human player should read from stdin", Symbol.ROCK, humanSymbol);
	}

	@Test
	public void testComputerPlayerShouldWorkWithNoHistory() {
		ComputerPlayer cp = new ComputerPlayer("name");
		Assert.assertNotNull("ComputerPlayer.chooseSymbol() should not return null if frequency does not apply",
				cp.chooseSymbol());
	}
